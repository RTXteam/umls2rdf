...
UMLS_LANGCODE_MAP = {"eng": "en", "fre": "fr", "cze": "cz", "fin": "fi", "ger": "de", "ita": "it",
                     "jpn": "jp", "pol": "pl", "por": "pt", "rus": "ru", "spa": "es", "swe": "sw",
                     "scr": "hr", "dut": "nl", "lav": "lv", "hun": "hu", "kor": "kr", "dan": "da",
                     "nor": "no", "heb": "he", "baq": "eu"}
...
def __get_connection():
    return MySQLdb.connect(host=conf.DB_HOST,
                           user=conf.DB_USER,
                           passwd=conf.DB_PASS,
                           db=conf.DB_NAME,
                           charset='utf8',
    ...
    def scan(self, filt=None, limit=None):
        ...
        while cont:
            if self.load_select:
                ...
                q = "SELECT * FROM %s WHERE %s LIMIT %s OFFSET %s" % (self.table_name,
                                                                      filt,
                                                                      self.page_size,
                                                                      page * self.page_size)
                if filt is None or len(filt) == 0:
                    q = "SELECT * FROM %s LIMIT %s OFFSET %s" % (self.table_name,
                                                                 self.page_size,
                                                                 page * self.page_size)
...
class UmlsClass(object):
    ...
    def getPrefLabel(self):
        if self.load_on_cuis:
            ...
            if len(self.rank) > 0:
                def sort_key(x):
                    return int(self.rank[self.rank_by_tty[x[MRCONSO_TTY]][0]][MRRANK_RANK])
...
class UmlsOntology(object):
    ...
    def load_tables(self, limit=None):
        ...
        for atom in mrconso.scan(filt=mrconso_filt, limit=limit):
            ...
            # self.atoms_by_code_ls.append(get_code(atom,self.load_on_cuis)) ???
            if not self.load_on_cuis:
    ...
    def terms(self):
        ...
        for code in self.atoms_by_code:
            ...
            ids = list(map(lambda x: x[field], code_atoms))
            rels = list()
            for _id in ids:
            ...
            if self.load_on_cuis:
                ...
                for rel in rels:
                    ...
                    if len(code_source) == 1 and len(code_target) == 1 and code_source[0] != code_target[0]:
            ...
            umls_class = UmlsClass(self.ns,
                                   atoms=code_atoms,
                                   rels=rels_to_class,
                                   defs=defs,
                                   atts=atts,
                                   rank=self.rank,
                                   rank_by_tty=self.rank_by_tty,
                                   sty=self.sty,
                                   sty_by_cui=self.sty_by_cui,
                                   load_on_cuis=self.load_on_cuis,
                                   is_root=is_root)
    ...
    def write_into(self, file_path, hierarchy=True):
        ...
        header_values = {'label': self.ont_code,
